...
const AntiSpamPage = () => {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                const { data } = await fetchPages({ slug: 'anti-spam' });
                if (data) setPage(data);
                const { data: copy } = await fetchCopyright();
                setCopyright(copy);
            } catch (error) {
                console.error("Failed to load anti-spam page:", error);
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div className="min-h-screen bg-background pb-12">
            <Helmet>
                <title>{page?.title || "Anti-Spam Policy"} | a2z Utility Hub</title>
                <meta name="description" content={page?.meta_description || "Read our anti-spam policy."} />
            </Helmet>

            <div className="bg-muted/30 border-b py-8">
                <div className="container">
                    <Breadcrumbs items={[{ title: "Legal" }, { title: "Anti-Spam", to: "/legal/anti-spam" }]} className="mb-4" />
                    <h1 className="text-4xl font-bold tracking-tight mb-2">{page?.title || "Anti-Spam Policy"}</h1>
            ...
            <div className="container py-8 grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <div className="lg:col-span-3">
                    <div 
                        ...
                        dangerouslySetInnerHTML={{ __html: page?.content || "<p>Our anti-spam policy is currently being updated. Please check back later.</p>" }} 
                    />
