...
const AdminTicketDetail = ({ ticket, onBack, onUpdate }) => {
  ...
  const fetchMessages = async () => {
    setLoading(true);
    // Manual join to avoid profiles:user_id syntax error
    ...
    const { data: profilesData, error: profilesError } = await supabase
        .from('profiles')
        .select('id, first_name, last_name, avatar_url, role_id')
        .in('id', userIds);
    ...
    const mergedMessages = msgsData.map(msg => ({
        ...msg,
        profiles: profileMap[msg.user_id] || {}
    }));
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[calc(100vh-140px)]">
      ...
      <div className="lg:col-span-2 flex flex-col h-full bg-card border rounded-xl overflow-hidden shadow-sm">
         ...
         <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar" ref={scrollRef}>
            ...
            {/* Thread */}
            {messages.map((msg) => {
               const isAdmin = msg.profiles?.role_id !== user.role_id && msg.user_id !== ticket.user_id; 
               // Better check: is message sender distinct from ticket creator?
               return (
      ...
      <div className="space-y-6 overflow-y-auto pr-1">
         ...
         {/* Internal Notes */}
         <AdminInternalNotes ticketId={ticket.id} />
      </div>
    </div>
  );
};
