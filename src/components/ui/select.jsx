...
const SelectTrigger = React.forwardRef(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-10 w-full items-center justify-between rounded-[12px] border border-gray-300 bg-white px-3 py-2 text-sm ring-offset-white transition-colors duration-150 ease-out",
...
const SelectContent = React.forwardRef(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        "relative z-50 min-w-[8rem] overflow-hidden rounded-[12px] border border-gray-200/50 bg-white/90 backdrop-blur-xl text-gray-900 shadow-elevation-2",
